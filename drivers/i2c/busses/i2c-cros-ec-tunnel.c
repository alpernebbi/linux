...
#define CHECK_I2C_WR(num, length) \
	(((i2c_msgs[num].flags & I2C_M_RD) == 0) && (i2c_msgs[num].len == length))

#define CHECK_I2C_RD(num, length) \
	((i2c_msgs[num].flags & I2C_M_RD) && (i2c_msgs[num].len == length))
...
static int ec_i2c_limited_xfer(struct i2c_adapter *adap, struct i2c_msg i2c_msgs[],
			       int num)
{
	...
	if ((num == 2) && CHECK_I2C_WR(0, 1) &&
		...
		/* real string length */
		i2c_msgs[1].buf[0] = strlen(tmpblock);
		strscpy(&i2c_msgs[1].buf[1], tmpblock, i2c_msgs[1].len);
		return ret < 0 ? ret : num;
	}
...
static int ec_i2c_probe(struct platform_device *pdev)
{
	...
	if (device_property_read_bool(dev, "google,limited-passthrough"))
